				...
				<view class="u-scroll-box" :class="{'u-tabs-scorll-flex': !isScroll}">
					<view class="u-tab-item u-line-1" :id="'u-tab-item-' + index" v-for="(item, index) in list" :key="index" @tap="clickTab(index)"
					 :style="[tabItemStyle(index)]">
						 <block v-if="!contentCustomShow">
							 <u-badge :count="item[count] || item['count'] || 0" :offset="offset" size="mini"></u-badge>
							 {{ item[name] || item['name']}}
						 </block>
						 <slot name="contentCustom" v-else />
					</view>
					<view v-if="showBar && !barCustomShow" class="u-tab-bar" :style="[tabBarStyle]"></view>
					<slot name="barCustom" v-else />
				</view>
			</scroll-view>
		</view>
	</view>
