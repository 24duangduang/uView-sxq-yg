	...
	<view class="">
		<movable-area class="u-swipe-action" :style="{ backgroundColor: bgColor }">
			...
			>
				...
				<view class="u-swipe-del" v-if="showBtn" @tap.stop="btnClick(item.buttonType ? item.buttonType : index)" :style="[btnStyle(item.style)]" v-for="(item, index) in options" :key="index">
					<button v-if="item.buttonType == 'share'" open-type="share" class="u-btn-text u-reset-button shareButton">{{item.text}}</button> 
					<view v-else class="u-btn-text">{{ item.text }}</view>
				</view>
			</movable-view>
		</movable-area>
	</view>
