	 ...
	 :class="'u-lazy-item-' + elIndex">
		<view :class="'u-lazy-item-' + elIndex">
			...
			<image :style="{	
				borderRadius: borderRadius + 'rpx',
				height: imgHeight,
				opacity: imgType == 'longImg' ? (loadStatus == 'loaded' ? '1' : '0') : '1',
				display: imgType == 'longImg' ? 'block;' : 'block;'
			}" 
			v-if="imgType == 'defaultImg' ? !isError : imgType == 'longImg'" class="u-lazy-item"
			 :src="isShow ? image : loadingImg" :mode="imgMode" @load="imgLoaded" @error="loadError" @tap="clickImg"></image>
			<image :style="{borderRadius: borderRadius + 'rpx', height: imgHeight,  display: imgType == 'longImg' ? (loadStatus == '!loaded' ? 'block' : 'none') : 'block'}"
			 v-if="isError&&imgType == 'defaultImg'||imgType == 'longImg'" class="u-lazy-item error" :src="errorImg"
